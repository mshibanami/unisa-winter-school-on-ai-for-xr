    ...
    public class ConvaiActionsHandler : MonoBehaviour
    {
        ...
        private void Reset()
        {
            actionMethods = new ActionMethod[]
            {
                ...
                new() { action = "Blue Zone", actionChoice = ActionChoice.BlueZone },
                new() { action = "Green Zone", actionChoice = ActionChoice.GreenZone },
                new() { action = "Yellow Zone", actionChoice = ActionChoice.YellowZone },
                new() { action = "Red Zone", actionChoice = ActionChoice.RedZone },
            };
        }
        ...
        private IEnumerator DoAction(ConvaiAction action)
        {
            ...
            switch (action.Verb)
            {
                ...
                case ActionChoice.None:
                    ...
                    break;
                case ActionChoice.BlueZone:
                    yield return BlueZone();
                    break;
                case ActionChoice.GreenZone:
                    yield return GreenZone();
                    break;
                case ActionChoice.YellowZone:
                    yield return YellowZone();
                    break;
                case ActionChoice.RedZone:
                    yield return RedZone();
                    break;
                default:
        ...
        private IEnumerator BlueZone()
        {
            ConvaiLogger.DebugLog("Blue Zone action triggered for Convai NPC: " + _currentNPC.name, ConvaiLogger.LogCategory.Actions);
            ActionStarted?.Invoke("Blue Zone", _currentNPC.gameObject);

            // float jumpForce = 5f;
            // GetComponent<Rigidbody>().AddForce(new Vector3(0f, jumpForce, 0f), ForceMode.Impulse);
            // _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Dance"), 1);
            yield return AnimationActions("BlueZone");

            ActionEnded?.Invoke("Blue Zone", _currentNPC.gameObject);
        }

        private IEnumerator GreenZone()
        {
            ConvaiLogger.DebugLog("Green Zone action triggered for Convai NPC: " + _currentNPC.name, ConvaiLogger.LogCategory.Actions);
            ActionStarted?.Invoke("Green Zone", _currentNPC.gameObject);

            // float jumpForce = 5f;
            // GetComponent<Rigidbody>().AddForce(new Vector3(0f, jumpForce, 0f), ForceMode.Impulse);
            // _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Dance"), 1);
            yield return AnimationActions("GreenZone");

            ActionEnded?.Invoke("Green Zone", _currentNPC.gameObject);
        }

        private IEnumerator YellowZone()
        {
            ConvaiLogger.DebugLog("Yellow Zone action triggered for Convai NPC: " + _currentNPC.name, ConvaiLogger.LogCategory.Actions);
            ActionStarted?.Invoke("Yellow Zone", _currentNPC.gameObject);

            // float jumpForce = 5f;
            // GetComponent<Rigidbody>().AddForce(new Vector3(0f, jumpForce, 0f), ForceMode.Impulse);
            // _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Dance"), 1);
            yield return AnimationActions("YellowZone");

            ActionEnded?.Invoke("Yellow Zone", _currentNPC.gameObject);
        }

        private IEnumerator RedZone()
        {
            ConvaiLogger.DebugLog("Red Zone action triggered for Convai NPC: " + _currentNPC.name, ConvaiLogger.LogCategory.Actions);
            ActionStarted?.Invoke("Red Zone", _currentNPC.gameObject);

            // float jumpForce = 5f;
            // GetComponent<Rigidbody>().AddForce(new Vector3(0f, jumpForce, 0f), ForceMode.Impulse);
            // _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Dance"), 1);
            yield return AnimationActions("RedZone");

            ActionEnded?.Invoke("Red Zone", _currentNPC.gameObject);
        }
